...
const TaskListItem: React.FC<TaskListItemProps> = ({ task, onRequestDelete }) => {
  ...
  return (
    <div
      className="grid grid-cols-[1fr_1fr_80px_40px] items-center gap-3 px-4 py-3 border-b border-claude-border/50 hover:bg-claude-surfaceHover/50 cursor-pointer transition-colors"
      onClick={() => dispatch(selectTask(task.id))}
    >
      {/* Title */}
      <div className={`text-sm truncate ${task.enabled ? 'text-claude-text' : 'text-claude-textSecondary'}`}>
        {task.name}
      </div>

      {/* Schedule */}
      <div className="text-sm text-claude-textSecondary truncate">
      ...
      <div className="flex justify-center">
        <div className="relative" ref={menuRef}>
          <button
            ...
            className="p-1.5 rounded-md text-claude-textSecondary hover:bg-claude-surfaceHover transition-colors"
          >
            <EllipsisVerticalIcon className="w-5 h-5" />
          </button>
          {showMenu && (
            <div className="absolute right-0 top-full mt-1 w-32 rounded-lg shadow-lg bg-white border border-claude-border z-50 py-1">
              <button
                type="button"
                onClick={handleRunNow}
                disabled={!!task.state.runningAtMs}
                className="w-full text-left px-3 py-1.5 text-sm text-claude-text hover:bg-claude-surfaceHover disabled:opacity-50"
              >
              ...
              <button
                type="button"
                onClick={handleEdit}
                className="w-full text-left px-3 py-1.5 text-sm text-claude-text hover:bg-claude-surfaceHover"
              >
              ...
              <button
                ...
                className="w-full text-left px-3 py-1.5 text-sm text-red-500 hover:bg-claude-surfaceHover"
              >
...
const TaskList: React.FC<TaskListProps> = ({ onRequestDelete }) => {
  ...
  if (tasks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 px-6">
        <ClockIcon className="h-12 w-12 text-claude-textSecondary/40 mb-4" />
        <p className="text-sm font-medium text-claude-textSecondary mb-1">
          {i18nService.t('scheduledTasksEmptyState')}
        </p>
        <p className="text-xs text-claude-textSecondary/70 text-center">
  ...
  return (
    <div>
      {/* Column Headers */}
      <div className="grid grid-cols-[1fr_1fr_80px_40px] items-center gap-3 px-4 py-2 border-b border-claude-border/50">
        <div className="text-xs font-medium text-claude-textSecondary">
        ...
        <div className="text-xs font-medium text-claude-textSecondary text-center">
